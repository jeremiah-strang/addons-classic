...
SUF.options.args.general.args.fontoutline = {
	type = "select",
	name = L["Overlay Font Outline"],
	desc = L["Text Overlay Font Outline"],
	get = function(info) return info.handler.db.global.overlayfont.fontoutline end,
	set = function(info, value)
		info.handler.db.global.overlayfont.fontoutline = value
		TO:RefreshFontStrings()
	end,
	values = {
		NONE = L["None"],
...
SUF.options.args.general.args.fontshadow = {
	type = "toggle",
	name = L["Overlay Font Shadow"],
	desc = L["Text Overlay Font Drop Shadow Effect"],
	get = function(info) return info.handler.db.global.overlayfont.fontshadow end,
	set = function(info, value)
		info.handler.db.global.overlayfont.fontshadow = value
		TO:RefreshFontStrings()
	end,
	width = "double",
	order = 130,
	...
	if unit then
		...
		if mappedUnit and frames[unit] then
			for text, fontString in pairs(frames[unit].fontStrings) do
				fontString:SetFont(LSM:Fetch("font", SUF.db.global.overlayfont.fontface), SUF.db.global.overlayfont.fontsize, SUF.db.global.overlayfont.fontoutline)
				if SUF.db.global.overlayfont.fontshadow then
